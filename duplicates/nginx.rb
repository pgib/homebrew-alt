...
class Nginx < Formula
  ...
  def options
    [
      ...
      ['--with-upload',    "Compile with the upload modules"]
    ]
  end

  def upload_config_args
    upload_module_path="/usr/local/src/nginx-upload-module"
    upload_progress_module_path="/usr/local/src/nginx-upload-progress-module"
    ...
    if !File.directory?(upload_progress_module_path)
      puts "Unable to install the nginx-upload-progress-module because #{upload_progress_module_path} was not found. Please run:"
      puts "mkdir -p /usr/local/src && cd /usr/local/src && git clone https://github.com/masterzen/nginx-upload-progress-module.git"
      exit
    end

    return [ "--add-module=#{upload_module_path}", "--add-module=#{upload_progress_module_path}" ]
    #return "--add-module=#{upload_module_path}" # --add-module=#{upload_progress_module_path}"
  end
